...
/*	node-sqlserver does not yet support multiple parameters
...
*/
    var query = "INSERT INTO " + this.table + " (LogDate, Level, Message) VALUES ('" + currentDate + "','" + level + "','" + msg + "')";
	sql.queryRaw(this.connectionString, query, function(err, results) {
		if (err) {
			self.emit('error', err);
		}
